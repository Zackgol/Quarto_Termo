    ...
    public partial class PaginaProdutos : Page
    {
        ...
        private void CarregarProdutosMySQL()
        {
            ...
            using (MySqlConnection conn = ConexaoMySQL.ObterConexao())
            {
                ...
                {
                    MessageBox.Show($"Erro MySQL ao carregar dados: {ex.Message}", "Erro de 
Banco", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
...
"Atenção", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            using (MySqlConnection conn = ConexaoMySQL.ObterConexao())
            {
                try
                {
                    ...
                    if (ehEdicao)
                    {
                        query = "UPDATE Produtos SET Nome = @nome, Preco = @preco, Estoque = @estoque 
WHERE Id = @id";
                    }
                    else
                    {
                        query = "INSERT INTO Produtos (Nome, Preco, Estoque) VALUES (@nome, @preco, 
@estoque)";
                    }
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        MessageBox.Show(ehEdicao ? "Produto atualizado com sucesso no MySQL!" : "Produto 
cadastrado com sucesso no MySQL!", 
...
MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void btnExcluir_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Selecione um registro na tabela para poder excluir.", "Aviso",
MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            var confirmacao = MessageBox.Show($"Deseja realmente remover o produto 
'{txtNome.Text}' ? ",
                                              "Confirmar Exclusão",
